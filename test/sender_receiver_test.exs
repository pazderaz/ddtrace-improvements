defmodule SenderReceiverTest do
  ...
  test "multiple timeouts result in no deadlocks" do
    {:error, :receiver_timeout} = Sender.send_ignored_message("This will time out")
    {:ok, _} = Sender.send_message("Testing recovery")
    {:error, :receiver_timeout} = Sender.send_ignored_message("This will also time out")

    {:error, :receiver_timeout} = Sender.send_ignored_message("Final time out")
  ...
  test "monitor shuts down via exit trapping", %{sender: s_pid, mon_sender: mon_pid} do
    ...
    spawn(fn -> Sender.send_crashing_timeout() end)
  ...
  test "normal message passing does not trigger deadlock notifications" do
    ...
    for _ <- 1..10 do
      assert {:ok, _} = Sender.send_message("Ping")
    end
  ...
  test "late reply after timeout does not break monitor state", %{sender: s_pid, receiver: r_pid} do
    ...
    delay = 10
    {:error, :receiver_timeout} = Sender.send_delayed_message(delay)
